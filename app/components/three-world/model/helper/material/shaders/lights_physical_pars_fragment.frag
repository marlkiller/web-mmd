...
// https://google.github.io/filament/Filament.md.html#materialsystem/anisotropicmodel/anisotropicspecularbrdf
	...
	float V_GGX_SmithCorrelated_Anisotropic( const in float roughness, const in float dotNV, const in float dotNL ) {

		float Gv = dotNL * (dotNV * (1.0 - roughness) + roughness);
		float Gl = dotNV * (dotNL * (1.0 - roughness) + roughness);
		return 0.5 / (Gv + Gl);

	}

	float D_GGX_Anisotropic( const in float dotTH, const in float dotBH, const in float dotNH, const in float roughness, const in float anisotropic ) {

		float aspect = inversesqrt(1.0 - anisotropic * 0.9);
		float ax = 1.0 / (roughness * aspect);
		float ay = aspect / roughness;

		float spec = pow2(dotTH * ax) + pow2(dotBH * ay) + dotNH * dotNH;
		spec = (ax * ay) / (spec * spec) * 1.0;
...
vec3 BRDF_GGX( const in vec3 lightDir, const in vec3 viewDir, const in vec3 normal, const in PhysicalMaterial material ) {
	...
	vec3 F = F_Schlick( f0, f90, dotVH );
		...
		float V = V_GGX_SmithCorrelated_Anisotropic( roughness, dotNV, dotNL );

		float D = D_GGX_Anisotropic( dotTH, dotBH, dotNH, roughness, material.anisotropy );
